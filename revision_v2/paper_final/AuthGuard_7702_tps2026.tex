...
\begin{itemize}
    ...
    \item \textbf{An audited benchmark and evaluation protocol, with a live temporal holdout.} AuthGuardBench-7702 combines 2,190 audited delegates in 790 families under family-disjoint cross-validation with external benign controls, an audited-implementation registry, and 752 independently collected live Ethereum delegates from a strictly later two-week window, disjoint at the family level. We release the protocol, split and family assignments, per-row predictions, donor provenance, and deterministic reconstruction scripts; redistribution of the third-party bytecode itself is pending license review, so we are careful to claim a reproducible evaluation pipeline rather than a distributable dataset.
...
\paragraph*{Where this model is and is not adequate} Score access with a bounded query budget is the right model for a screening \emph{service}, where the adversary observes verdicts but not weights. It is deliberately conservative for a screener shipped inside a wallet client, where a determined adversary can extract the parameters and mount white-box gradient attacks. We make this boundary explicit rather than eliding it: the results below are lower bounds on evadability for client-side deployment, and the strongest claim they support is a \emph{relative} one---how architectures compare when the attacker can search over bytecode edits. Closing the white-box case is necessary future work before a client-side deployment claim would be justified, and we do not make one.
...
\paragraph*{The mechanism, measured directly} The ablation establishes \emph{that} attention aggregation matters; it does not show \emph{why}. The design argument in Section~\ref{sec:system} is that attention can concentrate on the informative region while uniform weighting must dilute evidence in proportion to appended content. Because the model exposes its chunk weights, we can test this rather than infer it. For all 727 held-out positives, scored by each fold's own checkpoint, we recorded the fraction of attention mass falling on chunks composed entirely of appended donor bytes. Uniform weighting has no learned parameters, so its mass on appended chunks is exactly their share of the chunk count---the analytic baseline the chunk-mean control realizes empirically.

Figure~\ref{fig:dilution} shows the result. Under Flood-200\%, appended content occupies 63.1\% of chunks, but attention places only 37.6\% of its mass there, resisting 25.5 percentage points of dilution; the gap widens monotonically with flooding volume (8.2, 14.7, 22.1, and 25.5 points at 25\%, 50\%, 100\%, and 200\%). The mechanism is real and it is also \emph{partial}: attention still assigns more than a third of its weight to content that carries no signal, which is a direct explanation of why the residual attack success rate in Section~\ref{sec:rq2} is 0.181 rather than zero.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig-attention-dilution.pdf}
\caption{Attention mass landing on appended donor chunks, against the uniform-weighting baseline that mean pooling is forced to. All 727 held-out positives, each scored by its own fold's checkpoint. The shaded gap is the dilution the learned aggregation resists.}
\label{fig:dilution}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Attack success rate by target and strategy (727 sources, 64-query budget, $\le 2\times$ byte overhead, 100\% structural validity). Lower is better. Pooled ASR with family-clustered 95\% CI. Upper block: 3 seeds $\times$ 5 folds. Middle and lower blocks: seed 7702, 5 folds.}
\label{tab:asr}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
\midrule
\multicolumn{5}{@{}l}{\emph{With flooding-augmented training}} \\
\textbf{AuthGuard-Seq + aug.} & .839 & \textbf{.039} & \textbf{.051} & \textbf{.062} \\
 & & \tiny[.005,.085] & \tiny[.018,.096] & \tiny[.025,.112] \\
Flat CNN + aug. & .644 & .682 & .882 & .927 \\
 & & \tiny[.558,.788] & \tiny[.817,.936] & \tiny[.881,.967] \\
Hist.+4-gram XGB + aug. & .670 & .530 & .840 & .940 \\
 & & \tiny[.451,.610] & \tiny[.780,.894] & \tiny[.904,.969] \\
\midrule
\multicolumn{5}{@{}l}{\emph{Parameter-matched controls ($\approx$30K params, cf. Table~\ref{tab:ablation})}} \\
Chunk attention 16K & .847 & .045 & .068 & \textbf{.075} \\
 & & \tiny[.018,.083] & \tiny[.032,.119] & \tiny[.037,.126] \\
Flat control 16K & .831 & .422 & .705 & .889 \\
 & & \tiny[.355,.496] & \tiny[.639,.772] & \tiny[.851,.925] \\
Chunk mean 16K & .769 & .723 & .923 & .986 \\
 & & \tiny[.667,.781] & \tiny[.894,.950] & \tiny[.976,.994] \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph*{Random search beats guided search} On every target, seeded random search outperforms score-guided beam search (e.g. 0.181 versus 0.146 against AuthGuard-Seq, 0.986 versus 0.950 against XGBoost). The beam follows first-step score reductions into a local optimum while random search explores more diverse compositions on the same budget. We report the \emph{maximum} over strategies as the attack result, so a weak guided search cannot flatter the defence: the reported ASR is the best of two budget-matched attackers, and it is the undirected one that is stronger here. This also means the numbers in Table~\ref{tab:asr} should be read as lower bounds on what a better-designed search would achieve.
...
Under a single fixed transformation---the protocol used throughout prior bytecode security learning, and in the earlier version of this work---a 15-feature logistic regression and a hierarchical neural model are \textbf{indistinguishable}. The separation appears only under adaptive search and grows monotonically with attack strength, as Figure~\ref{fig:strength} shows. This is the strongest justification we can offer for the methodology itself: it is not decoration around a model result, it is the only instrument in this study that resolves the comparison.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig-attack-strength.pdf}
\caption{Attack success against attacker strength. The models that are close under a fixed transformation separate under budgeted adaptive search.}
\label{fig:strength}
\end{figure}

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{fig-clean-vs-asr.pdf}
\caption{Clean detection and worst-case attack success for every evaluated configuration, ordered by robustness. Clean detection spans a narrow band and does not order the models; attack success spans almost the full range. The three configurations with attention aggregation occupy the robust end regardless of parameter count or training scheme.}
\label{fig:cleanvsasr}
\end{figure*}

\paragraph*{Can training substitute for architecture?} The natural objection to an architectural claim is that robustness might come from training data rather than design: augment the baselines with flooded examples and the gap may close. We tested this directly. Each model was retrained with its training set doubled by one flooding-augmented copy of every training contract, drawn from train-partition donors with fractions cycled over 25--200\% so the augmented set spans the flooding range. Results are the lower block of Table~\ref{tab:asr}.

Augmentation does help---against the transformation it was trained on. Under fixed Flood-200\% it cuts Flat CNN from 0.750 to 0.682 and XGBoost from 0.677 to 0.530. Under adaptive search it barely moves: Flat CNN 0.945 to 0.927, XGBoost 0.986 to 0.940. Both augmented baselines remain worse than clean-trained AuthGuard-Seq by $+0.774$ (CI $[+0.697,+0.841]$) and $+0.786$ (CI $[+0.703,+0.852]$) respectively, intervals excluding zero. Augmentation therefore does not substitute for the architecture. It is also not free for the baselines: Flat CNN loses clean detection (0.722 to 0.644) while buying almost no adaptive robustness---the familiar robustness--accuracy tension.

The same intervention behaves completely differently on the attention model. AuthGuard-Seq falls from 0.181 to \textbf{0.062} under adaptive search ($-0.133$, CI $[-0.194,-0.086]$) at essentially no clean cost (0.843 to 0.839). Augmentation and attention aggregation are complementary rather than alternative: the augmented data helps when the architecture can exploit it. We note this is also a third instance of the paper's methodological point---judged only on fixed Flood-200\%, augmentation would look like a substantial defence for the baselines, and under adaptive evaluation it is not.

\paragraph*{The mechanism claim holds under adaptive attack} Section~\ref{sec:rq1} attributed robustness to attention aggregation using clean and fixed-flooding evaluation. Since we argue fixed transformations are an insufficient instrument, we re-tested that attribution with the adaptive attack on the same parameter-matched controls (lower block of Table~\ref{tab:asr}). At roughly 30{,}000 parameters each, chunk attention reaches 0.075 adaptive ASR against 0.889 for the flat control and 0.986 for chunk mean---a thirteenfold gap to mean pooling at matched capacity, with paired contrasts of $+0.837$ (CI $[+0.775,+0.884]$) and $+0.710$ (CI $[+0.636,+0.772]$) against AuthGuard-Seq. The mechanism attribution is therefore not an artefact of the weaker protocol.

One result runs against expectation and we report it as such: the 30{,}050-parameter matched attention control is \emph{more} robust than the 63{,}266-parameter deployed model ($-0.129$, CI $[-0.189,-0.081]$) at equal clean detection (0.847 versus 0.843). On this evidence the smaller matched configuration, not the larger reference architecture, is the one a deployment should use.
...
The deployment-relevant figure is the authorization-weighted one, because a wallet's users encounter delegates in proportion to how often they are actually authorized, not in proportion to how many distinct delegates exist. On that basis the nominal 5\% point flags \textbf{9.4\%} of live authorization volume and the nominal 1\% point flags \textbf{5.9\%}; the latter is a plausible deployment setting for an advisory tier. The weighted rates are roughly half the per-contract rates because high-volume delegates are predominantly popular wallet-infrastructure implementations that score low, while the long tail of rarely-authorized delegates carries most of the flags.

Two further readings are required. First, thresholds drift substantially: the nominal 5\% point flags 20.3\% of distinct live delegates, far above the 6.5\% seen on curated benign controls, so population-specific recalibration is mandatory before deployment. Second, this stream is real mixed traffic containing genuinely risky delegates, so 20.3\% is a flag rate, not a false-positive rate, and cannot be decomposed without labels---we deliberately do not report it as a false-alarm measurement.
...
The timed artifact is the seed-7702, fold-0 cross-validation checkpoint, used here for cost measurement rather than as a released model. A deployed screener would be a single model retrained on all folds with calibration and thresholds fit on a held-out split drawn from the intended deployment population, following the recalibration finding of Section~\ref{sec:rq3}; we report cross-validation artifacts throughout because every accuracy and robustness number in this paper is a held-out estimate, and a fully retrained model would have no untouched data left to evaluate it on.
...
\paragraph*{Trust establishment under delegated execution} EIP-7702 changes what a signature means. Before it, authorizing a transaction committed a user to one state transition they could in principle inspect; after it, authorizing a delegation commits them to whatever a third-party program does with their account until revoked. That shifts the security question from transaction validation to \emph{counterparty assessment}, and it does so at a moment when the usual bases for trust---history, reputation, audit status, verified source---are frequently absent by construction, because the delegate may have been deployed minutes earlier. Bytecode screening is best understood as a substitute trust signal for exactly that gap: it cannot establish that a delegate is safe, but it can supply evidence where no other evidence exists yet, and it can do so in milliseconds at the moment the decision is made. This framing also explains why adversarial robustness is the property that matters. A trust signal an adversary can cheaply manufacture is worse than no signal, because it converts absence of warning into false assurance. The same argument applies to any learned trust signal over attacker-authored artifacts, which is why we regard the evaluation methodology rather than the specific model as the transferable contribution.
...
\emph{Variance.} Three training seeds inform the primary clean and adaptive-attack results. The augmented-training and parameter-matched control blocks of Table~\ref{tab:asr} are single-seed (7702) across all five folds, so their confidence intervals reflect family resampling but not seed variability; the effects there are large relative to the seed spread observed elsewhere (at most 0.020 in fold-then-seed ASR), but we do not claim seed-level stability for them. Elsewhere in this project, cuDNN algorithm selection moved a comparable recall metric by up to 0.026, so reported deviations may understate cross-hardware variability.
