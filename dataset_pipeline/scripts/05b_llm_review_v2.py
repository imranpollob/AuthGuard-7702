...
data/llm_reviews/{run_id}_promptv2/ so the two can be compared.
"""
...
from lib.llm_review_rubric_v2 import MODEL_ID, PROMPT_VERSION, review_evidence_packet  # noqa: E402
...
def main():
    ...
    out_dir = os.path.join(cfg["_resolved_paths"]["llm_reviews"], f"{run_id}_promptv2")
    os.makedirs(out_dir, exist_ok=True)

    v1_index_path = os.path.join(cfg["_resolved_paths"]["llm_reviews"], f"{run_id}_review_index.csv")
    ...
    index_path = os.path.join(cfg["_resolved_paths"]["llm_reviews"], f"{run_id}_review_index_promptv2.csv")
    ...
    if v1 is not None:
        summary["label_counts_v1"] = df["v1_label"].value_counts().to_dict()
        summary["n_label_changed_v1_to_v2"] = int((df["v1_label"] != df["proposed_label"]).sum())
        summary["transition_matrix_v1_to_v2"] = (
            df.groupby(["v1_label", "proposed_label"]).size().unstack(fill_value=0).to_dict()
        )
        for subset, mask in [
            ("v1_low_confidence", df["v1_confidence"] == "low"),
            ("partial_coverage", df["coverage_status"] == "PARTIAL"),
            ("v1_R1", df["v1_label"] == "R1"),
        ]:
            sub = df[mask]
            summary[f"subset_{subset}"] = {
                "n": int(len(sub)),
                "n_changed": int((sub["v1_label"] != sub["proposed_label"]).sum()),
                "v2_labels": sub["proposed_label"].value_counts().to_dict(),
            }
    print(json.dumps(summary, indent=2, default=str))
    with open(os.path.join(cfg["_resolved_paths"]["llm_reviews"], f"{run_id}_review_summary_promptv2.json"), "w") as f:
...
def _row(r, parsed, packet, v1, out_path):
    ...
    if v1 is not None and r.address in v1.index:
        row["v1_label"] = v1.loc[r.address, "proposed_label"]
        row["v1_confidence"] = v1.loc[r.address, "confidence"]
    return row
